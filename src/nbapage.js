...
function NbaPage() {
  ...
  useEffect(() => {
    const today = getSportsToday().toISOString().split("T")[0];
  ...
  useEffect(() => {
    ...
    const fetchGames = async () => {
      setLoading(true);
      try {
        await updateAvailableDays(weekOffset);

        const events = await fetchGamesForDay(selectedDay);
        const NbaGames = events.map((event) => {
          ...
          if (comp.odds?.length > 0) {
            ...
            spread = odds.spread ? `${odds.spread > 0 ? "+" : ""}${odds.spread}` : "-";
              ...
              return (
                <tr key={i}>
                  <td>
                    ...
                    />
                    {g.homeTeam}
                    {isLive && <span className="live-indicator" title="Live"></span>}
                  </td>
                  <td>{g.status}</td>
                  <td>{g.awayScore}</td>
                  <td>{g.homeScore}</td>
